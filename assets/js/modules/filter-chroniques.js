document.addEventListener('DOMContentLoaded', () => {
    ...
    const filters = ['filter-genre', 'filter-theme', 'filter-nation']
    ...
    const getFiltered = () => {
        const [genre, theme, nation] = filters.map(f => f.value);
        return posts.filter(post => {
            const pGenre = post.dataset.genrePrincipal;
            const pThemes = post.dataset.themes?.split(' ') || [];
